...
Section headers appear above each group on the dashboard. When sorting by Gainers, Losers, A–Z, or `|BX|`, headers are hidden and all cards are sorted globally; switching back to Default restores the sectioned layout. `|BX|` sorts by absolute BX high→low (null/insufficient BX sinks to the bottom).
